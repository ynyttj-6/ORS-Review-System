...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!["returned", "redevelop"].includes(existing.status) || !existing.reviews[0]) throw new ApiError(409, "该选品当前不可提交异议");
    const product = await db.$transaction(async (tx) => {
      await tx.objection.create({ data: { productId: id, roundId: existing.reviews[0].id, submitterId: developer.id, content: input.content } });
      const updated = await tx.product.update({ where: { id }, data: { status: "objection_pending" }, include: productInclude });
    ...
    if (product.reviewerId) await notifyUser({ recipientId: product.reviewerId, event: "objection", title: "选品异议待复审", content: `选品 **${product.name}** 的开发人员提交了异议，请复审。`, link: `${process.env.NEXT_PUBLIC_APP_URL}/review`, color: "orange" });
