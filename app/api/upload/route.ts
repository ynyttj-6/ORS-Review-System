...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE) throw new ApiError(400, "单个附件不能超过 1MB");
    ...
    if (product._count.attachments >= 10) throw new ApiError(409, "每个选品最多上传 10 个附件");
    ...
    try {
      const attachment = await db.attachment.create({ data: { productId, uploaderId: user.id, fileName: file.name, filePath: path, fileSize: file.size, fileType: file.type } });
      ...
      return ok({ id: attachment.id, name: attachment.fileName, size: attachment.fileSize, type: attachment.fileType, path: attachment.filePath }, 201);
    } catch (error) {
